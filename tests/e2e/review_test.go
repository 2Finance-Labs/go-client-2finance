...
func TestReviewFlow(t *testing.T) {
	...
	// assert.Equal(t, reviewer.PublicKey, updatedReview.Reviewer)
	// assert.Equal(t, reviewee.PublicKey, updatedReview.Reviewee)
	...
	// assert.WithinDuration(t, newStart, updatedReview.StartAt, time.Second)
	// assert.WithinDuration(t, newExp, updatedReview.ExpiredAt, time.Second)
	...
	// assert.Equal(t, reviewer.PublicKey, hiddenReview.Reviewer)
	// assert.Equal(t, reviewee.PublicKey, hiddenReview.Reviewee)
	// assert.Equal(t, subjectType, hiddenReview.SubjectType)
	// assert.Equal(t, subjectId, hiddenReview.SubjectID)
	// assert.Equal(t, 4, hiddenReview.Rating)
	// assert.Equal(t, "Updated comment", hiddenReview.Comment)
	// assert.Equal(t, "4", hiddenReview.Tags["quality"])
	// assert.Equal(t, "bafy2", hiddenReview.MediaHashes[0])
	...
	// assert.Equal(t, 1, reviewState5.Reports)
