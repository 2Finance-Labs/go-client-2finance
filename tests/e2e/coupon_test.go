...
func TestCouponFlow(t *testing.T) {
	...
	if cp.Address == "" { t.Fatalf("coupon addr empty") }
	...
	if _, err := c.UpdateCoupon(cp.Address, tok.Address, couponV1Domain.DISCOUNT_TYPE_FIXED, "", amt(2, dec), amt(10, dec), start2, exp2, false, 10, 2, pcHash2); err != nil { t.Fatalf("UpdateCoupon: %v", err) }
	...
	if _, err := c.GetCoupon(cp.Address); err != nil { t.Fatalf("GetCoupon: %v", err) }
	if _, err := c.ListCoupons("", tok.Address, "", nil, 1, 10, true); err != nil { t.Fatalf("ListCoupons: %v", err) }
}